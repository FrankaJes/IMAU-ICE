...
!    CALL sync
    ...
    DO j = 1, grid%ny
      ...
      IF (ice%mask_cf_a( j,i) == 1 .AND. ice%mask_shelf_a( j,i) == 1) THEN
        dVi_MB( j,i) = (SMB%SMB( j,i) + BMB%BMB( j,i)) * grid%dx * grid%dx * dt * ice%float_margin_frac_a( j,i)
      ELSEIF (ice%mask_ocean_a( j,i) == 1 .AND. ice%mask_shelf_a( j,i) == 0) THEN
        dVi_MB( j,i) = 0._dp
      ELSE
        dVi_MB( j,i) = (SMB%SMB( j,i) + BMB%BMB( j,i)) * grid%dx * grid%dx * dt
      END IF
  ...
  SUBROUTINE calc_dHi_dt_semiimplicit( grid, ice, SMB, BMB, dt)
    ...
    IF (par%master) THEN
      ...
      ! ==================================================================
        ...
        IF (ice%mask_cf_a( j,i) == 1 .AND. ice%mask_shelf_a( j,i) == 1) THEN
          MB_net = (SMB%SMB( j,i) + BMB%BMB( j,i)) * ice%float_margin_frac_a( j,i)
        ELSEIF (ice%mask_ocean_a( j,i) == 1 .AND. ice%mask_shelf_a( j,i) == 0) THEN
          MB_net = 0._dp
        ELSE
          MB_net = (SMB%SMB( j,i) + BMB%BMB( j,i))
        END IF
